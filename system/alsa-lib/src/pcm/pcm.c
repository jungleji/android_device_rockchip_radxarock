...
int snd_pcm_hw_params_set_flags(snd_pcm_t *pcm, snd_pcm_hw_params_t *params, unsigned int val)
{
	params->flags = val;
	return 0;
}
				      ...
				      snd_pcm_xfer_areas_func_t func)
{
	...
	_again:
		...
		if ((state == SND_PCM_STATE_RUNNING &&
            ...
            //timeout:3s
            //in somecase wait will never return,so write will never return.
            //so,AudioFlinger will be blocked.
			err = snd_pcm_wait(pcm, 3000);
			if (err < 0)
				break;
			goto _again;			
		}
